...
export const createShipments = async(req:Request,res:Response, next:NextFunction)=>{
    try {
...
export const getShipmentsById = async(req:Request,res:Response, next:NextFunction)=>{
    try {
        const {id} = req.params;
...
export const updateShipments = async(req:Request,res:Response, next:NextFunction)=>{
    try {
        const {id} = req.params;
        const result = await ShipmentDTO.safeParse(req.body);
        if(!result.success){
...
export const patchShipments = async(req:Request,res:Response, next:NextFunction)=>{
    try {
        const {id} = req.params;
        const shipments = await Shipments.findById(id);
        if(!shipments){
